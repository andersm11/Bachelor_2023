...
class Reconstructor:
    ...
    def get_n_variance(self,S, variance):
    ...
    def get_pca(self,patches :np.ndarray , window_size:int, variance=1.0, n_componets = 0):
        '''
        ...
        self.got_pca = True
        if(variance == 1.0):
            self.n_var = len(self.Eig)
        else:
            #self.n_var = self.get_n_variance(pca.explained_variance_ratio_ , variance)
            self.n_var = n_componets
        return (self.Vecs[:self.n_var,:] , self.Eig[:self.n_var], self.mu)
    ...
    def reconstruct_image(self, img, n_components, window_size):
        ...
        for i in range(X-window_size):
            for j in range(Y-window_size):
                ...
                recon_patch = self.reconstruct_patch(patch_row, n_components)
